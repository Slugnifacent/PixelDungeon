...
public:
	...
	AKRESULT FindObbPath(JNIEnv* lJNIEnv, jobject& in_jNativeActivity);
...
public:
	...
	AKRESULT InitAndroidIO(
		JavaVM* java_vm ///< The Java VM pointer
		, jobject& in_jNativeActivity ///< The NativeActivity, as given by android_app->activity->clazz (or any other method).
		);

	/// Sets the base path for file loading (banks and streamed files).  This path must be inside the app's APK or OBB, relative to the root of the archive.  The same path is used for both APK and OBB.
